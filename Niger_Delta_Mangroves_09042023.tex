...
The Enhanced Vegetation Index (EVI) uses NIR, Red and Blue bands and is designed to quantify vegetation greenness. Its extended formula includes the atmospheric conditions and considers the effects from the canopy background. The EVI is well adjusted to identifies areas with dense vegetation which is suitable for mangrove mapping \cite{HUETE2002195}. Its values are ranging from -1 to +1, similar to the NDVI, with the diapason indicating healthy vegetation lying between 0.2 and 0.8. \hl{The computational formula of EVI is presented in the Equation }\ref{eq13}:
